...
Write-Host "[+] Creating Namespace Manager"
...
Write-Host "[+] Quering for GPOs to Build GPO to GUID Table"
...
Write-Host "[+] Adding Extension Namespaces to the Namespace Manager"
foreach ($group in $extensionGroups)
{
	...
	foreach ($extension in $group.Extensions)
	{
		...
		foreach ($key in $extensionNamespace.Keys)
		{
			...
			{
				Write-Host "[+] Added $($key) with Namespace $($extensionNamespace.$key) to the Manager"
...
if ($output)
{
	...
	Write-Host "[+] Exporting Results to CSV: $($output)\$($ComputerName)_$($nameOfReport)_$($dateOfReport).csv"
...
if ($PassThru)
{
	...
	Write-Host "[+] Writing results to console"
	return $results
}
